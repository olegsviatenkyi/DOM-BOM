function addEmployee() {
	...
	var limitText = "the limit of the number of employees was reached";
	var limitAvgSalaryText = "the limit of the avg. salary was reached";
	var number = document.querySelector(".number");
	var resultSalary = document.querySelector(".salary");
	
	function addUlLiSpan(){
		...
		if (firstName.match(/[a-zA-Z]/)){
			...
			if(employeesNumber < limit){
				...
				if(checkingFirstName() === -1){
				if(checkingLastName() === -1){
					newUl.appendChild(newLi);
					newLi.appendChild(newSpan1);
					newLi.appendChild(newSpan2);
					newLi.appendChild(newSpan3);
					newLi.appendChild(newSpan4);
			}else alert("An employee with that last name already exists")		
			}else alert("An employee with that first name already exists")

			if (avgSalary () > limitAvgSalary){
                newUl.removeChild(newUl.lastChild);
                alert(limitAvgSalaryText);
      }

			}else {alert(limitText)}
